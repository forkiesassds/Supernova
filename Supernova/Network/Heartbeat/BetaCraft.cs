...
namespace Supernova.Network {
    ...
    /// <summary> Heartbeat to BetaCraft.pl's web server. </summary>
    public sealed class BetaCraftBeat : Heartbeat {
        string proxyUrl;
        ...
        public override void OnResponse(string response) {
            ...
            if (response == Server.URL) return;
            Server.URL = response;
            
            if (!response.Contains("\"errors\":")) {
                Server.UpdateUrl(Server.URL);
                File.WriteAllText("text/externalurlbetacraft.txt", Server.URL);
                Logger.Log(LogType.SystemActivity, "BetaCraft URL found: " + Server.URL);
            } else {

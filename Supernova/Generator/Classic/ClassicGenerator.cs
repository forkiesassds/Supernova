...
namespace ClassicalSharp.Generator {
    ...
    public sealed partial class ClassicGenerator {
        ...
        public static void RegisterGenerators() {
            Supernova.Generator.MapGen.Register("Island3", Supernova.Generator.GenType.Classic, GenIsland, defHelp);
            Supernova.Generator.MapGen.Register("Floating", Supernova.Generator.GenType.Classic, GenFloating, defHelp);
            Supernova.Generator.MapGen.Register("Flat2", Supernova.Generator.GenType.Classic, GenFlat, defHelp);
            Supernova.Generator.MapGen.Register("Inland", Supernova.Generator.GenType.Classic, Gen, defHelp);
        }

        static bool Gen(Player p, Level lvl, string seed) {
            int seed_ = Supernova.Generator.MapGen.MakeInt(seed);
            new ClassicGenerator().Generate(lvl, seed_, "normal");
            return true;
        }
        static bool GenIsland(Player p, Level lvl, string seed) {
            int seed_ = Supernova.Generator.MapGen.MakeInt(seed);
            new ClassicGenerator().Generate(lvl, seed_, "island");
            return true;
        }
        ...
        public byte[] Generate(Level lvl, int seed, string type) {
            blocks = lvl.blocks;
            Width  = lvl.Width;
            Height = lvl.Height;
            Length = lvl.Length;
            ...
            FloodFillWaterBorders(type);
            FloodFillWater(type);
            FloodFillLava(type);

            CreateSurfaceLayer(type);
            PlantFlowers();
            PlantMushrooms();
            PlantTrees();
            return blocks;
            ...
            if (type == "flat") {
                ...
                for (int z = 0; z < Length; z++) {
                    for (int x = 0; x < Width; x++) {
                        ...
                        height *= 0.5;
                        if (type == "island") {
                            double abs = Math.Abs((x / (Width - 1.0) - 0.5) * 2.0);
                            double abs2 = Math.Abs((z / (Length - 1.0) - 0.5) * 2.0);
                            double n6 = Math.Sqrt(abs * abs + abs2 * abs2) * 1.2000000476837158;
                            if ((n6 = Math.Max(n6 = Math.Min(n6, n2.Compute(x * 0.05f, z * 0.05f) / 4.0 + 1.0), Math.Max(abs, abs2))) > 1.0) {
                                n6 = 1.0;
                            }
                            if (n6 < 0.0) {
                                n6 = 0.0;
                            ...
                            n6 *= n6;
                            if ((height = height * (1.0 - n6) - n6 * 10.0 + 5.0) < 0.0) {
                                height -= height * height * 0.20000000298023224;
                            }
                        }
                        else if (height < 0) height *= 0.8f;
        ...
        void CreateStrata(string type) {
            OctaveNoise n = new OctaveNoise(8, rnd);
            OctaveNoise n2 = new OctaveNoise(8, rnd);
            ...
            for (int z = 0; z < Length; z++) {
                double abs3 = Math.Abs((z / (Length - 1.0) - 0.5) * 2.0);
                for (int x = 0; x < Width; x++) {
                    double max = (max = Math.Max(abs3, Math.Abs((x / (Width - 1.0) - 0.5) * 2.0))) * max * max;
                    double n4;
                    int height2;
                    if ((height2 = (int)((height2 = (int)(Math.Sqrt(Math.Abs(n4 = n2.Compute(x * 2.3, z * 2.3) / 24.0)) * Math.Sign(n4) * 20.0) + waterLevel) * (1.0 - max) + max * Height)) > waterLevel) {
                        height2 = Height;
                    }
                    ...
                    mapIndex = 0 * oneY + z * Width + x;
                    for (int y = 0; y < height2; y++) {
                        if (type == "floating") blocks[mapIndex] = Block.Air; mapIndex += oneY;
                    }
                }
            }
        }
        ...
        void FloodFillWaterBorders(string type) {
        ...
        void CreateSurfaceLayer(string type) {
            ...
            for (int z = 0; z < Length; z++) {
                for (int x = 0; x < Width; x++) {
                    ...
                    byte blockUnder = y > 0 ? blocks[index - oneY] : Block.Air;
                    if ((blockAbove == Block.Water || (type == "floating" && blockAbove == Block.Air)) && (n2.Compute(x, z) > 12)) {
                        blocks[index] = ((blockUnder == Block.Air) ? Block.Gravel : ((blockAbove == Block.Water) ? Block.Gravel : Block.Grass));
                    } else if (blockAbove == Block.Air) {
                        blocks[index] = (y <= waterLevel && (n1.Compute(x, z) > 8)) ? ((type != "flat") ? Block.Sand : Block.Grass) : ((blockUnder == Block.Air) ? Block.Air : Block.Grass);
                    }
                }
            }
        }

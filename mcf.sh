...
mcfdir="./"							##  Folder containing Supernova.exe
...
if [ ! -f "${mcfdir}/Supernova.exe" ]; then
	echo -e "cannot find ${mcfdir}/Supernova.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcfdir}/Supernova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${mcfdir}/Supernova.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${mcfdir}/Supernova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcfdir}/Supernova.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${mcfdir}/Supernova.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${mcfdir}/Supernova_.update" ]; then
			if [ -f "${mcfdir}/Supernova.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${mcfdir}/Supernova.exe
				rm ${mcfdir}/Supernova_.dll
				mv ${mcfdir}/Supernova.update ${mcfdir}/Supernova.exe
				mv ${mcfdir}/Supernova_.update ${mcfdir}/Supernova_.dll
				if [ -f "${mcfdir}/Supernova_.update" ]; then
					if [ -f "${mcfdir}/Supernova.update" ]; then
						echo -e FAILED!
						if [ -f "${mcfdir}/Supernova_.dll" ]; then
							if [ -f "${mcfdir}/Supernova.exe" ]; then
								$0 ${default}
							fi
						else
